...
# ---------- Tab 今日关注 ----------
with tab_daily:
    st.markdown("**只列『三周期共振』(月线强 + 周线顺 + 日线均线多头)且当天刚突破放量的龙头,按热度排序。其余一律不列 —— 每天只看这一屏。**")
    dtk = load_universe()
    ...
    if dres is not None:
        if len(dres) == 0:
            ...
            ddf = pd.DataFrame(dres).head(15).rename(columns={
                "ticker":"代码","heat":"热度","price":"价$","rsi":"日RSI","mrsi":"月RSI","vsurge":"放量x",
                "dist52":"离52高","trig":"今日触发","stop":"止损$","target":"目标$"})
            st.dataframe(ddf[["代码","热度","价$","日RSI","月RSI","放量x","离52高","今日触发","止损$","目标$"]],
...
- **周线:** 金叉 + 站上周MA30(顺周线)。逆周线(死叉)长期净亏。
- **日线:** 均线多头(MA10>20>30>50)+ RSI 55–82 + 当天突破放量。RSI 40–60 是你的死亡区。
